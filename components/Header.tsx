...
const NavLink: React.FC<{ href: string; children: React.ReactNode; onLinkClick?: () => void; isActive: boolean }> = ({ href, children, onLinkClick, isActive }) => (
  <a 
    href={href} 
    onClick={(e) => {
    ...
    className="relative group block md:inline-block px-3 py-2 rounded-md text-base font-medium transition-colors duration-300"
  >
...
const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    { href: '#other-experience', label: 'Other Experience' },
      ...
      {isOpen && (
        <div className="md:hidden bg-black/50 backdrop-blur-lg" id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
             {navLinks.map(link => (
                <NavLink key={link.href} href={link.href} isActive={activeSection === link.href.substring(1)} onLinkClick={closeMenu}>
                  {link.label}
                </NavLink>
              ))}
          </div>
